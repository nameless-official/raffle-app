    ...
    this.operationsDatagrid.set({
      ...
      delete: (record: Record<string, string | number | boolean>) => {
        ...
        this.confirmationService.confirm({
          ...
          rejectLabel: 'No',
          accept: () => {
            this.RaffleStatusService.deleteRecord(Number(record.uomt_id)).subscribe((responseData) => {
              if (!responseData) return
              this.loadData()
              this.messageService.add({
                severity: 'success',
                summary: '¡Registro eliminado con éxito!',
                detail: `El registro ${record.uomt_name} ha sido eliminado exitosamente`
              });
