...
import { SortOptionList, SortValue } from '../../interfaces/sort-option-list.interface';
...
export class RafflesComponent {
  ...
  onTextFilter(eventText: any): void {
    ...
    this.rafflesService.getTotalRecordsByConditions(conditions).pipe(
      switchMap((responseData: number) => {
        if (responseData === 0) return of([])

        this.totalRecords.set(responseData);

        return this.rafflesService.searchRecords(conditions)
  ...
  onDeleteData(raffleData: Raffle): void {
    this.confirmationService.confirm({
      ...
      rejectLabel: 'No',
      accept: () => {
        this.rafflesService.deleteRecord(Number(raffleData.raffle_id)).subscribe((responseData) => {
